...
const sendMessageCentralSMS = require("../utils/sendMessageCentralSMS");
...
router.post("/signup", async (req, res, next) => {
  try {
    ...
    await sendMessageCentralSMS(
      phoneNumber.startsWith("+")
        ? phoneNumber.slice(1)
        : `91${phoneNumber.replace(/^\+/, "")}`,
      `Welcome to नव धारावी! Your login password is: ${plainPassword}`
    );
  ...
  try {
    ...
    await sendMessageCentralSMS(
      phoneNumber.startsWith("+")
        ? phoneNumber.slice(1)
        : `91${phoneNumber.replace(/^\+/, "")}`,
      `Your new नव धारावी login password is: ${newPlainPassword}`
    );
